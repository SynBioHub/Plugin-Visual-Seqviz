...
import nonDNATypes from require('./nonDNATypes');


function renderSegment(design, segment, interactions, interaction) {
    ...
    var glyphs = segment.sequence.map(function (glyphObject) {
        ...
        if (design.proportional && glyphObject.start !== glyphObject.end) {
        ...
        if (glyphObject.isComposite) {
            ...
            compositeGlyph.transform({
                matrix: Matrix.toSVGString(compositeMatrix)
            });
        }
        ...
        if (glyphObject.name !== undefined) {
            ...
            var length = 10 //max length of string
            //if label is too long, truncate it and add "..."
            if (glyphObject.name.length > length) {

                glyphObject.name = glyphObject.name.substring(0, length) + "...";
            }
            var label;
            ...
            labelText.transform({
                matrix: Matrix.toSVGString(labelMatrix)
            });
        }
        ...
        if (segment.role === "inhibition_nonDNA") {
            ...
            if (segment.participantRole !== "inhibitor") {
                var arrowPos = Matrix();
                var arrowTransform = Matrix.rotate(arrowPos, 180, {
                    x: 20,
                    y: 20
                });
                arrowGlyph.transform({
                    matrix: Matrix.toSVGString(arrowTransform)
                });
            }
        ...
        else if (segment.role === "stimulation_nonDNA") {

            var arrowProps = Arrows.StimulatorArrow.render(design, glyphObject, boundingBoxSize);
            var arrowGlyph = arrowProps.glyph;

            //if role of nonDNA part is not stimulator flip the direction of simulation arrow
            //biologically it might never be the case
            if (segment.participantRole !== "stimulator") {
                var arrowPos = Matrix();
                var arrowTransform = Matrix.rotate(arrowPos, 180, {
                    x: 24,
                    y: 27
    ...
    if (!isNonDNA) {
        if (segment.topologies.includes("circular")) {
            ...
            backbone.transform({
                matrix: Matrix.toSVGString(Matrix.translation(Vec2(-design.geom.segmentPadding, 0)))
            });
        } else {
    ...
    if (segment.isCircuit) {
        ...
        box.transform({
            matrix: Matrix.toSVGString(Matrix.translation(boundingBox.topLeft))
        });
    ...
    if (segment.name !== undefined) {
        ...
        labelText.transform({
            matrix: Matrix.toSVGString(Matrix.translation(Vec2(-100, -100)))
        });
